...
behind the compiler, see [docs/METHODOLOGY.md](docs/METHODOLOGY.md). For the
July 2026 debug benchmark, see [docs/debug-bench-2026-07-03.md](docs/debug-bench-2026-07-03.md).

### Compiler fidelity enhancements

Recent compiler work adds deterministic validation and generation paths on top
of the core capture-to-code pipeline:

| Capability | Description |
| --- | --- |
| **Dual deliverable** | Generated Next.js app at `/` plus static HTML mirror at `/static/` |
| **Frozen live witness** | `source/evidence/live-witness/` — HTML, DOM, and screenshots captured once; gates never re-hit production |
| **Extended gates** | HTML witness (2b), DOM witness triangle (3b/3c), manifest hash (6b), responsive probes, visual audit |
| **Lottie replay** | Captured animation JSON → `DittoLottie` client component; static frame SVG for gates |
...
| **Job events** | Postgres `job_events` table streams clone progress for REST `/events` polling |
...
| `profiles/` | site-specific benchmark acceptance tiers |
